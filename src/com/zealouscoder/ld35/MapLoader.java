        ...
        readMap.getLayers().forEach((layer) -> {
            if (layer instanceof ObjectGroup) {
                ...
                objs.forEach((obj) -> {
                    ...
                    objProps.put("SpawnPosition", GamePosition.wrap(obj.getX(), obj.getY(), game.getMapViewLayer()));
                    GenericGameObject go = new GenericGameObject("obj" + obj.getX() + ":" + obj.getY(), obj.getType(),
                            null, objProps);
                    game.add(go);
                ...
                for (int x = 0; x < w; ++x) {
                    for (int y = 0; y < h; ++y) {
                        ...
                        if (t != null) {
                            Sprite sprite = new Sprite(images.get(t.getId()), t.getWidth(), t.getHeight(),
                                    GamePosition.wrap(x * t.getWidth(), y * t.getHeight(), layerView));
                            GenericGameObject go = new GenericGameObject("tile" + x + ":" + y, "tile" + t.getId(),
                                    sprite, t.getProperties());
                            game.add(go);
                        }
                    }
                }
